...
public class DataDrivenManager extends ExcelManager
{
	...
	public final static int TEST_NAME_COLUMN =1;
	...
	public final static int TEST_DATA_STRAT_COLUMN=2;
	...
	public Object[][] getTestCaseDataSets(String sheetName, String testName)
	{
		...
		int testRowNumber = getRowNumber(sheetName, TEST_NAME_COLUMN, testName);
		int testDataStartRow = testRowNumber +1;
		
		// Calculate test data row count
		int testDataRows = 0;
		for(int i=testDataStartRow; getCellData(sheetName, TEST_NAME_COLUMN, i).equals(testName); i++ )
		{
			testDataRows++;
		}
		
		// Calculate test data column count
		int testDataCols = getCellCount(sheetName, testRowNumber) - TEST_DATA_STRAT_COLUMN+1;
		...
		for(int i=0;i<testDataRows;i++)
		{
			...
			{
				testCaseDataSets[i][j] = getCellData(sheetName, TEST_DATA_STRAT_COLUMN+j, testDataStartRow+i);
			}
		}
